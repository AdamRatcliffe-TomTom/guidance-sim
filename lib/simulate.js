 ...
 * @param {boolean} config.updateCamera If `true` the map's camera will be updated
...
function simulate(map, config, version) {
  ...
  var updateCamera = config.updateCamera;
  ...
  var options = {
    ...
    frequency: frequency,
    response: response,
    updateCamera: config.updateCamera !== undefined ? config.updateCamera : true
  };

  // Start an interval with default speed, and run `setMap` function
  var interval = util.requestInterval(function () {
...
function setMap(interval, speed, options) {
  ...
  if (step == null) {
    ...
    options.response.emit(util.clearRequestInterval(interval));
  } else {
    step["speed"] = util.distanceConvert(step["speed"], "mi"); // convert speed to miles per hour
    var modified = getManeuverParams(
      options.config,
      options.version,
      step,
      options.maneuvers
    ...
    options.response.emit("update", {
      pitch: pitch,
      zoom: zoom,
      stepCoords: step.coords,
      stepBearing: step.bearing,
      stepTime: step.time,
    ...
    if (version === "v5") {
      if (maneuvers[key][2] === min) {
        ...
        for (var i = 0; i < config.maneuvers.length; i++) {
          if (
            (config.maneuvers[i].type.indexOf(type) >= 0 &&
              config.maneuvers[i].modifier.indexOf(modifier) >= 0) ||
            (config.maneuvers[i].type.indexOf(type) >= 0 &&
              config.maneuvers[i].modifier == false)
          ) {
...
function calculateParams(config, i, min, property) {
  ...
  var response =
    config[property] +
    percentToManeuver * (config.maneuvers[i][property] - config[property]);
  return response;
}
